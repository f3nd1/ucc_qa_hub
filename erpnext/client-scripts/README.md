...
2. For this record's Criterion, paste the **GD4 requirement** and the
   **Procedure/SOP** text (or paste a Google Doc link and click **Pull from
   Drive**). These are cached in your browser per criterion, so you only do it
   once per criterion. Optionally click **Fetch available models** to pick the
   OpenAI model. Save grounding.
